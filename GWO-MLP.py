...
epoch =25
pop_size = 25
gwo_model = OriginalGWO(epoch=epoch, pop_size=pop_size)
best_agent = gwo_model.solve(problem)
...
output_file = 'regression_results_GWO_Torch.csv'
main_df.to_csv(output_file, index=False)
print(f"\n📁 پیش‌بینی انجام شد و نتیجه در '{output_file}' ذخیره شد.")

# 📊 رسم نمودار فقط max R² تا هر epoch
plt.figure(figsize=(10,6))
plt.plot(range(1, len(r2_max_history)+1), r2_max_history, marker='o', color='red')
plt.xlabel("Epoch")
plt.ylabel("Max R² Score (Train)")
plt.title("MLP optimized by GWO")
plt.grid(True)
...
plt.tight_layout()
plt.savefig("FinalModel_MLP_GWO.png")
plt.show()
